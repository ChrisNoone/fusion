...
from Backstage.page import Page
...
class Test(TestCase):
    ...
    def test_fail_registered_01(self):
        ''' 注册用户 '''
        # 指定某一个用户登陆
        self.page = Page(self.base_driver)
        # 登陆
        self.page.login()
        # 点击充值
        self.page.chongzhi()
        ...
        # 打开csv文件
        csv_file = open('/fusion/csv/test.csv', 'r', encoding='utf8')
        # 读取csv文件
        ...
        for row in csv_date:
            if row['msg'] == 'fail01':
                ...
                self.base_driver.wdraw = Withdraw(self.base_driver)
                self.base_driver.wdraw.withdraw(row)
                self.base_driver.wdraw.withdraw_button()
                self.base_driver.forced_wait(1)
                get_tips01 = self.base_driver.wdraw.get_tips01()
                # 断言，断言方法在TestCase类中
                # 获取在登录成功之后的用户名
